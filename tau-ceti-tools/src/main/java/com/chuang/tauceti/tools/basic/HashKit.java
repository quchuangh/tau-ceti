...
public class HashKit {
    ...
    public static byte[] des(byte[] str, byte[] pwd) {
        try{
    ...
    public static String desHex(String str, String pwd, String charset) throws UnsupportedEncodingException {
        return HexKit.encodeHexStr(des(str.getBytes(charset), pwd.getBytes(charset)));
    }
    ...
    public static String encode(String algorithmName, String origin) {
        ...
        try {
            ...
            resultString = HexKit.encodeHexStr(md.digest(origin
                    .getBytes()));
        } catch (Exception ex) {
    ...
    public static String sha1Hex(byte[] source, byte[] salt, int hashIterations) {
        try {
            return HexKit.encodeHexStr(encode(SHA_1, source, salt, hashIterations));
    ...
    public static String encodeHex(String algorithmName, byte[] source, byte[] salt, int hashIterations) {
        try {
            return HexKit.encodeHexStr(encode(algorithmName, source, salt, hashIterations));
        } catch (NoSuchAlgorithmException e) {
            logger.error("", e);
            return "";
        }
    ...
    public static String sha256_HMAC(String message, String secret) {
        ...
        try {
            ...
            hash = HexKit.encodeHexStr(bytes);
        } catch (Exception e) {
    ...
    public static String signSrc(Map<String, ? extends Object> map, String separator, String eq, boolean useEmpty, int order) {
        ...
        for(String key: keys) {
            ...
            src.append(key).append(eq).append(value.toString()).append(separator);
        }
    ...
    public static String signSrcValueOnly(Map<String, ? extends Object> map, String separator, boolean useEmpty, int order) {
        ...
        for(String key: keys) {
            ...
            Object value = map.get(key) == null ? "" : map.get(key);
            src.append(value.toString()).append(separator);
        }
        return src.substring(0, src.length() - separator.length());
    }
}
